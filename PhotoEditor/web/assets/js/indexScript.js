  ...
  let dTool = document.querySelectorAll('.dTool');
...
let style = "#000"
dTool.forEach((dtool,index) => {
	dtool.addEventListener("click" , function() {
		if(index==0){
			...
			canvas.addEventListener('mousemove', e => {
				if (isDrawing === true) {
					lineDraw(context,x,y,e.clientX - rect.left,e.clientY - rect.top,style);
					x = e.clientX - rect.left;
					y = e.clientY - rect.top;
				}
			});
